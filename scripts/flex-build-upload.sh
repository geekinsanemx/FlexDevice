...
# Version: 1.0.0
# Changelog:
#   1.0.0 - Adapted from flex-fsk-tx's scripts/flex-build-upload.sh for FlexDevice's
#           layout (sketch under flex-fsk-tx-v2.5/, src/version.h single source of
#           truth). No --enable-* module flags: FlexDevice has no compile-time
#           optional subsystems (RTC_ENABLED is currently a hardcoded config.h
#           value, not a build-flag toggle).
# Usage:
#   ./flex-build-upload.sh [-t ttgo|heltec] [-p /dev/ttyUSBX] [-u] [-e] [--skip-prereqs] sketch.ino
#   ./flex-build-upload.sh [-t ttgo|heltec] [-p /dev/ttyUSBX] [-u] [-e] [--skip-prereqs] backup-file
...
function check_prerequisites() {
    ...
    local required_libs=("RadioLib" "U8g2" "ArduinoJson" "RTClib")
...
if [[ $# -lt 1 ]]; then
    echo "Usage: $0 [-t ttgo|heltec] [-p /dev/ttyUSBX] [-u] [-e] [--skip-prereqs] sketch.ino"
    exit 1
fi

PARSED_ARGS=$(getopt -o t:p:ue -l type:,port:,upload,erase,skip-prereqs -- "$@") || {
    echo "Usage: $0 [-t ttgo|heltec] [-p /dev/ttyUSBX] [-u] [-e] [--skip-prereqs] sketch.ino"
    exit 1
}
